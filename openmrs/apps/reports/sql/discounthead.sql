select acc.name as "Free Patients Categories",so.care_setting as "Free Patients Type",sum(discount_amount) as "Discount Amount",
count(so.id) as "Free Patients" from sale_order so inner JOIN
