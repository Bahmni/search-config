...
(SELECT concat(round((sum(isHTInControl('#startDate#','#endDate#',  pid))/count(pid))*100,2), ' %') as Percent_of_Patient_Visit_with_hypertension_incontrol
from (SELECT person_id as pid from confirmed_diagnosis_view_new
...
(SELECT concat(round((sum(isHTInControl('#startDate#','#endDate#',  pid))/count(pid))*100,2), ' %') as Percent_of_Distinct_Patient_with_hypertension_incontrol
from (SELECT person_id as pid from confirmed_diagnosis_view_new
